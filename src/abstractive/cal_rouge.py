...
def process(data):
    candidates, references, pool_id, rouge_dir = data
    cnt = len(candidates)
...
def test_rouge(cand, ref,num_processes, rouge_dir=None):
    ...
    if num_processes == 0:
        return process((candidates, references, 0, rouge_dir))
    else:
        ...
        for i in range(n_pool):
            arg_lst.append((candidates_chunks[i],references_chunks[i],i,rouge_dir))
        pool = Pool(n_pool)
