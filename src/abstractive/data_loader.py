...
class AbstractiveBatch(object):
    ...
    def __init__(self, data=None, hier=False, pad_id=None, device=None, is_test=False, shuffle_order=False):
        ...
        if data is not None:
            ...
            if (hier):
                ...
                # Adding the order parameter and shuffling the order of the 
                if shuffle_order:
                    para_order = []
                    for ind, x in enumerate(data):
                        order = list(range(max_ntoken))
                        random.shuffle(order)
                        para_order.append(order)
                        tmp = src[ind][0]
                        for idx,i in enumerate(order):
                            tmp[i] =  src[ind[0][idx]]
                        src[ind][0] = tmp
                            
                    para_order = torch.tensor(para_order).transpose(0,1)
                    setattr(self, 'para_order', para_order.to(device))
            ...
            ## adding query part 
            if len(data[0][3])!=0:
                query = [x[3] for x in data]
                _query = self._pad(query, width=max([len(d) for d in query]), height=len(query), pad_id=pad_id)
                query = torch.tensor(_query[0]) # batch_size, q_len
                setattr(self, 'query', query.to(device))
...
class AbstracticeIterator(object):
    ...
    def preprocess(self, ex):
        ...
        """ adding query seperately"""
        """ **Test Pass**: N/A"""
        if self.args.query:
            query = ex['query']
            # append the query to every paragraph
            if self.args.model_type in ['query', 'heq', 'hero']:
                src = [p for p in src]
            else:
                src = [ query + p  for p in src]
        else:
            query = []
        ...
        if (not self.args.hier):
            ...
            return src, tgt, tgt_str, query

        src = [p[:self.args.trunc_src_ntoken] for p in src]

        return src[:self.args.trunc_src_nblock], tgt, tgt_str, query
