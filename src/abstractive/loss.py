...
class LossComputeBase(nn.Module):
    ...
    def sharded_compute_loss(self, batch, output,
        ...
        Args:
          ...
          output (:obj:`FloatTensor`) :
              output of decoder model `[tgt_len x batch x hidden]`
          c_attn (dict) : dictionary of attention related losses "discourse" and "coverage"
              `[tgt_len x batch x src_len]`
...
class PTLossCompute(LossComputeBase):
    """
    Pointer Networks Loss Computation
    """
    def __init__(self):
        super(PTLossCompute, self).__init__(generator=None, pad_id=-100)
        self.criterion = nn.NLLLoss()

    def _make_shard_state(self, batch, output):
        return {
            "output": output,
            "target": batch.para_order
        }
