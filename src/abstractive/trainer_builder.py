...
from abstractive.loss import PTLossCompute
...
class Trainer(object):
    ...
    def validate(self, valid_iter):
        ...
        with torch.no_grad():
            for batch in valid_iter:
                src = batch.src
                tgt = batch.tgt
                outputs, _, attn = self.model(src, tgt)
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            ...
            if self.args.model_type in ['query', 'heq', 'hero']:
                query = batch.query
                outputs, _, attn = self.model(src, tgt, query)
            else:
                outputs, _, attn = self.model(src, tgt)
    ...
    def _save(self, step, score):
        ...
        checkpoint_path = os.path.join(self.args.model_path, 'model_step_%f_%d.pt' % (score, step))
        logger.info("Saving checkpoint %s" % checkpoint_path)
        # checkpoint_path = '%s_step_%d.pt' % (FLAGS.model_path, step)
        if (not os.path.exists(checkpoint_path)):
            torch.save(checkpoint, checkpoint_path)
            # check number of checkpoints
            paths = glob(os.path.join(self.args.model_path, '*.pt'))
            if len(paths)>self.args.max_num_checkpoints:
                delete_step = int(paths[0].split("_")[-1].replace(".pt",""))
                delete_score = float(paths[0].split("_")[-2].replace(".pt",""))
                for path in paths[1:]:
                    mstep = int(path.split("_")[-1].replace(".pt",""))
                    mscore = float(path.split("_")[-2].replace(".pt",""))
                    if mscore<delete_score:
                        delete_step = mstep
                        delete_score = mscore
                # delete the oldest model\
                remove_path = os.path.join(self.args.model_path, "model_step_%f_%d.pt" % (delete_score, delete_step))
